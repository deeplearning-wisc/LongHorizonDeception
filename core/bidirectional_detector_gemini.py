...
class BidirectionalDetector:
    ...
    def _call_llm_with_retry(self, context_detector_input: str, global_round: int) -> Dict[str, Any]:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                # Create fresh LLM client for this round (using Gemini detector config)
                ...
                print(f"\033[96m[DETECTOR] Generated detection for round {global_round} (Gemini)\033[0m")
...
def main():
    ...
    # Hardcode detector to use Gemini (OpenRouter) profile
    config_handler_for_api = ConfigHandler()
    api_profiles = config_handler_for_api._load_api_profiles()

    if 'gemini_2_5_pro' not in api_profiles['api_profiles']:
        raise ValueError("FATAL: gemini_2_5_pro not found in api_profiles.yaml")

    detector_config = api_profiles['api_profiles']['gemini_2_5_pro'].copy()

    print(f"Detector model: gemini-2.5-pro (hardcoded)")
    
    # Initialize detector with atomic parallel architecture
    # Enforce OpenRouter call behavior: no max_tokens, no transforms
    detector_config['omit_max_tokens'] = True
